...
(defun start-ros-node (name &key (xml-rpc-port 8001) (pub-server-port 7001) 
  ...
  (let ((params (handle-command-line-arguments name)))
    (unless master-supplied
      (let ((env-master-uri (sb-ext:posix-getenv "ROS_MASTER_URI")))
	(when env-master-uri
	  (mvbind (address port) (parse-uri env-master-uri)
	...
	(setf address (lookup-hostname-ip-address address))))
      
    (ros-info "Using master URI ~a" master-uri)
...
(defun shutdown-ros-node ()
  ...
  (with-recursive-lock (*ros-lock*)
	...
	(protected-call-to-master ("unregisterPublisher" topic *xml-rpc-caller-api*) c
	  (warn "Could not contact master when unregistering as publisher of ~a during shutdown: ~a" topic c))
	...
	(protected-call-to-master ("unregisterSubscriber" topic *xml-rpc-caller-api*) c
	    (warn "Could not contact master when unsubscribing from ~a during shutdown: ~a" topic c))
    ...
    (do-hash (name s *services*)
      (let ((i (protected-call-to-master ("unregisterService" name *service-uri*) c
		   (warn "During shutdown, unable to contact master to unregister service ~a: ~a" name c)
		   1)))
	(unless (eql i 1)
...
(defun advertise (topic topic-type)
  ...
  (with-fully-qualified-name topic
	...
	(error "Unable to contact master for advertising ~a: ~a" topic c)))))
...
(defun register-service-fn (service-name function service-type)
  ...
  (with-fully-qualified-name service-name
	...
	(protected-call-to-master ("registerService" service-name uri *xml-rpc-caller-api*) c
	  ...
	  (error "Socket error ~a when attempting to contact master for advertising service ~a" c service-name))))))
    ...
    (with-mutex (*ros-lock*)
	  ...
	  (let ((sub (make-subscription :buffer (make-queue :max-size max-queue-length) 
	    ...
	    (update-publishers topic
				 ...
				 (error "Could not contact master when registering as subscriber to ~a: ~a" topic c)))
	    (values))))))
...
(defun get-param (key &optional (default nil default-supplied))
  (with-fully-qualified-name key
	...
	(protected-call-to-master ("getParam" key) c
	    (error "Could not contact master when getting param ~a: ~a" key c))
	(if default-supplied
	    default
...
(defun set-param (key val)
  (with-fully-qualified-name key
    (protected-call-to-master ("setParam" key val) c
      (error "Could not contact master when setting param ~a" key))))

(defun has-param (key)
  (with-fully-qualified-name key
    (protected-call-to-master ("hasParam" key) c
      (error "Could not contact master for call to hasParam ~a: ~a" key c))))

(defun delete-param (key)
  (with-fully-qualified-name key
    (protected-call-to-master ("deleteParam" key) c
      (error "Could not contact master when deleting param ~a: ~a" key c))))
