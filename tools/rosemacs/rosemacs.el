...
(defun update-ros-topic-buffer-helper ()
  ...
  (princ (format "\nSubscribed topics:\n") ros-topic-buffer)
  (dolist (topic ros-subscribed-topics)
    (princ (format " %s" topic) ros-topic-buffer)
    (terpri ros-topic-buffer)))
...
(defun parse-rostopic-list (process event)
  ...
  (unwind-protect
	  ...
	  (save-excursion
		...
		(when start
		  (let ((end (re-search-forward "Subscribed topics:")))
		    ...
		    (let ((published-topics (get-topics start end)))
		      (setq ros-subscribed-topics (get-topics end (point-max)))
		      (destructuring-bind (added deleted) (diffs ros-topics published-topics)
	...
	(lwarn '(rosemacs) :debug "rostopic process received unexpected event %s." event))
