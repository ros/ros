...
macro(rosbuild_init)
  ...
  #
  rosbuild_invoke_rospack(gtest _gtest INCLUDE_DIRS cflags-only-I)
  include_directories(${_gtest_INCLUDE_DIRS})
  rosbuild_invoke_rospack(gtest _gtest LIBRARY_DIRS libs-only-L)
  link_directories(${_gtest_LIBRARY_DIRS})
  rosbuild_invoke_rospack(gtest _gtest temp cflags-only-other)
  _rosbuild_list_to_string(_gtest_CFLAGS_OTHER "${_gtest_temp}")
  rosbuild_invoke_rospack(gtest _gtest LIBRARIES libs-only-l)
  rosbuild_invoke_rospack(gtest _gtest temp libs-only-other)
  _rosbuild_list_to_string(_gtest_LDFLAGS_OTHER "${_gtest_temp}")
  #
  # The following code removes duplicate libraries from the link line,
  # saving only the last one.
  #
...
macro(rosbuild_add_library lib)
  ...
  if(ROS_BUILD_STATIC_LIBS)
    ...
    foreach(_src ${_srclist})
      ...
      # Set up for gcov
      _rosbuild_add_gcov(${_src} ${static_lib_name})
    endforeach(_src)
    ...
    # Attach compile and link flags
    rosbuild_add_compile_flags(${static_lib_name} ${${PROJECT_NAME}_CFLAGS_OTHER})
    rosbuild_add_link_flags(${static_lib_name} ${${PROJECT_NAME}_LDFLAGS_OTHER})
    # Link lib against dependent libs
    target_link_libraries(${static_lib_name} ${${PROJECT_NAME}_LIBRARIES})

    # Add ROS-wide compile and link flags (usually things like -Wall).  These
    # are set in rosconfig.cmake.
    rosbuild_add_compile_flags(${static_lib_name} ${ROS_COMPILE_FLAGS})
    rosbuild_add_link_flags(${static_lib_name} ${ROS_LINK_FLAGS})
    ...
    rosbuild_add_compile_flags(${static_lib_name} -fPIC)
