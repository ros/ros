...
PUBLISHERS : list of publishers, each of which is a list (ADDRESS PORT)."
  ...
  (force-format *log* "~&Publisher update ~a ~a" topic publishers)
  (with-mutex (*ros-lock*)
...
2. This call does not actually set up the transport over the agreed-upon protocol.  In the TCP case, the subscriber must then connect to the given address and port over TCP, and send a string containing the topic name and MD5 sum."
  ...
  (with-mutex (*ros-lock*)
	...
	(if (hash-table-has-key *publications* topic)
	    ...
	    ;; TCPROS-specific
	    (list 1 (format nil "ready on ~a:~a" (hostname) *publication-server-port*)
		  (list "TCPROS" *publication-server-address-string* *publication-server-port*))
...
(defun update-publishers (topic publishers)
  ...
  (force-format *log* "~&update-publishers for topic ~a with publishers ~a" topic publishers)

  (when (stringp publishers)
    (assert (= 0 (length (string-trim '(#\Space #\Tab #\Newline) publishers))) nil
	    "In update publishers, got a string ~a rather than a list of publishers" publishers)
    (setf publishers nil))
...
Right now, the transport must be TCPROS and the return value is the socket."
  ...
  (force-format *log* "~&Subscribe ~a ~a" uri topic)
  (unless (hash-table-has-key *subscriptions* topic) (error "I'm not subscribed to ~a" topic))

  (dbind (protocol address port) (ros-rpc-call uri "requestTopic" topic (list (list "TCPROS")))
    (assert (string= protocol "TCPROS") nil "Protocol ~a did not equal TCPROS" protocol)
