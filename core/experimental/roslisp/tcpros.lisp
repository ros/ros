	...
	(mvbind (sub known) (gethash topic *subscriptions*)
	  ...
	  (let ((buffer (buffer sub))
	    ...
	    (send-tcpros-header str 
				"topic" topic 
				"md5sum" (string-downcase (format nil "~x" (md5sum topic))) 
				"type" (ros-datatype topic)
				"callerid" (fully-qualified-name *ros-node-name*))
	    (let ((response (parse-tcpros-header str)))

	      (when (assoc "error" response :test #'equal)
		(error "During TCPROS handshake, publisher sent error message ~a" (cdr (assoc "error" response :test #'equal))))
...
(defun parse-tcpros-header (str)
  (let ((remaining-length (deserialize-int str))
	...
	(assert (>= remaining-length 0) nil 
		"Error parsing tcpros header: header length and field lengths didn't match")
	(let ((tokens (tokens field-string :separators '(#\=))))
	  (assert (= 2 (length tokens)) nil "Tokens in header field ~a did not have length 2" field-string)
	  (push (cons (first tokens) (second tokens)) key-value-pairs))))
    key-value-pairs))
